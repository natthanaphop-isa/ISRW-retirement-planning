  ...
  return (
    <div className="card space-y-6 bg-white sticky top-4 max-h-[95vh] overflow-y-auto">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold text-blue-950">{t.app_title}</h2>
      ...
      <div className="space-y-4">
        <h3 className="font-semibold text-blue-900">{t.personal_info}</h3>
        
        <div>
          <label className="flex justify-between text-sm text-slate-700 mb-1">
            <span>{t.current_age}</span>
            <span className="font-medium text-blue-950">{inputs.current_age}</span>
          </label>
        ...
        <div>
          <label className="flex justify-between text-sm text-slate-700 mb-1">
            ...
            <span className="font-medium text-blue-950">{inputs.retirement_age}</span>
          </label>
        ...
        <div>
          <label className="flex justify-between text-sm text-slate-700 mb-1">
            ...
            <span className="font-medium text-blue-950">{inputs.life_expectancy}</span>
          </label>
      ...
      <div className="space-y-4">
        <h3 className="font-semibold text-blue-900">{t.accumulation}</h3>
        <div>
          <label className="block text-sm text-slate-700 mb-1">{t.initial_capital}</label>
          <FormattedInput className="input-base text-blue-950"
            value={inputs.initial_capital} onChange={(val) => update('initial_capital', val)} />
        </div>
        <div>
          <label className="block text-sm text-slate-700 mb-1">{t.monthly_investment}</label>
          <FormattedInput className="input-base text-blue-950"
      ...
      <div className="space-y-4">
        <h3 className="font-semibold text-blue-900">{t.retirement_spending}</h3>
        <div>
          <label className="block text-sm text-slate-700 mb-1">{t.monthly_need}</label>
          <FormattedInput className="input-base text-blue-950"
            value={inputs.monthly_need} onChange={(val) => update('monthly_need', val)} />
        </div>
        <div>
          <label className="block text-sm text-slate-700 mb-1">{t.monthly_want}</label>
          <FormattedInput className="input-base text-blue-950"
      ...
      <div className="space-y-4">
        <label className="flex items-center space-x-2">
          ...
          <span className="font-semibold text-blue-900 text-sm">{t.health_costs}</span>
        </label>
        {inputs.include_health_costs && (
          <div className="pl-6 space-y-3">
            <div>
              <label className="block text-xs text-slate-700 mb-1">{t.health_insurance}</label>
              <FormattedInput className="input-base text-blue-950"
                value={inputs.health_insurance_lump_sum} onChange={(val) => update('health_insurance_lump_sum', val)} />
            </div>
            <div>
              <label className="block text-xs text-slate-700 mb-1">{t.health_other}</label>
              <FormattedInput className="input-base text-blue-950"
      ...
      <div className="space-y-4">
        <label className="flex items-center space-x-2">
          ...
          <span className="font-semibold text-blue-900 text-sm">{t.extra_costs}</span>
        </label>
        {inputs.include_extra_costs && (
          <div className="pl-6 space-y-3">
            <div>
              <label className="block text-xs text-slate-700 mb-1">{t.extra_lump_sum}</label>
              <FormattedInput className="input-base text-blue-950"
      ...
      {/* Section 7: Economic */}
      <div className="space-y-4">
        <h3 className="font-semibold text-blue-900">{t.economic}</h3>
        
        <div>
          <label className="flex justify-between text-sm text-slate-700 mb-1">
            <span>{t.inflation}</span>
            <span className="font-medium text-blue-950">{(inputs.inflation_rate * 100).toFixed(1)}%</span>
          </label>
        ...
        <div>
          <label className="flex justify-between text-sm text-slate-700 mb-1">
            ...
            <span className="font-medium text-blue-950">{(inputs.expected_return_accumulation * 100).toFixed(1)}%</span>
          </label>
        ...
        <div>
          <label className="flex justify-between text-sm text-slate-700 mb-1">
            ...
            <span className="font-medium text-blue-950">{(inputs.expected_return_near_retirement * 100).toFixed(1)}%</span>
          </label>
          <input type="range" min="0" max="0.10" step="0.005" className="range-slider"
        ...
        <div>
          <label className="flex justify-between text-sm text-slate-700 mb-1">
            ...
            <span className="font-medium text-blue-950">{inputs.glide_path_years}</span>
          </label>
        ...
        <div>
          <label className="flex justify-between text-sm text-slate-700 mb-1">
            ...
            <span className="font-medium text-blue-950">{(inputs.expected_return_post_retirement * 100).toFixed(1)}%</span>
          </label>
      ...
      <div className="space-y-4">
        <label className="flex items-center space-x-2">
          ...
          <span className="font-semibold text-blue-900 text-sm">Enable Monte Carlo Simulation</span>
        </label>
        
        {inputs.enable_monte_carlo && (
          <details className="mt-2">
            <summary className="text-sm font-semibold text-slate-700 cursor-pointer">{t.advanced_mc}</summary>
            <div className="pt-4 space-y-4 pl-2">
              <div>
                <label className="flex justify-between text-xs text-slate-700 mb-1">
                  <span>{t.n_sims}</span>
                  <span className="font-medium text-blue-950">{inputs.n_simulations}</span>
                </label>
              ...
              <div>
                <label className="flex justify-between text-xs text-slate-700 mb-1">
                  ...
                  <span className="font-medium text-blue-950">{(inputs.sigma_accum * 100).toFixed(1)}%</span>
                </label>
