...
current_age = st.slider("อายุปัจจุบัน (ปี)", 15, 65, 30, 1)
retirement_age = st.slider("อายุเกษียณ (ปี)", 30, 75, 65, 1)
life_expectancy = st.slider("อายุขัย (ปี)", 50, 100, 85, 1)
starting_principal = st.number_input("เงินทุนตั้งต้น (฿)", 0, 10000000, 100000, 100000)
...
if selection1: 
    health_insurance_expense = st.number_input("[HEALTH] เบี้ยประกันสุขภาพ หลังวัยเกษียณจนสิ้นอายุขัย (฿)", 0, 10000000, 3000000, 100000)
    health_risk_expense = st.number_input("[HEALTH] เงินก้อนค่าใช้จ่ายอื่น ๆ ด้านสุขภาพ ณ วันเกษียณ (฿)", 0, 10000000, 1000000, 100000)
else:
...
if selection2: 
    etc_expense = st.number_input("[EXTRA] เงินก้อนค่าใช้จ่ายพิเศษอื่น ๆ ณ วันเกษียณ", 0, 10000000, 300000, 100000)
else:
    etc_expense = 0
...
st.markdown(f"Facebook: https://www.facebook.com/isarawealth.th")
st.markdown(f"Website: www.isarawealth.com")
